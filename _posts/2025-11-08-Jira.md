...
# Dejo dos esqueletos para scripts en bash, para interactuar con la api de Jira
...
PROJECT_KEY="${PROJECT_KEY:-ABK}"      # clave del proyecto JSM, ej. ABK
OUT_FILE="${OUT_FILE:-abk_keys.txt}"   # salida con issueKeys (una por línea)
...
echo "⬇️  Listando requests… (puede tardar)" >&2
while :; do
  R="$(
    curl -sS -u "${JIRA_EMAIL}:${JIRA_TOKEN}" -H "Accept: application/json" \
...
if [[ -n "$ISSUE_KEY" ]]; then
  ...
  if (( len == 0 )); then
    echo "ℹ️ No hay comentarios en este ticket."
    exit 0
  fi
  ...
  echo "🕓 Δ Creación → 1º:          ${diff1_min} min (${diff1_hr} h)"
  echo ""
  ...
  if (( len >= 2 )); then
    ...
    echo "🕓 Δ 1º → 2º:                ${diff2_min} min (${diff2_hr} h)"
    ...
    echo ""
    echo "ℹ️ No hay segundo comentario en este ticket."
  fi
