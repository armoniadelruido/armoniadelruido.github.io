...
```bash
export JIRA_SITE="example.atlassian.net"
export JIRA_EMAIL="usuario@example.com"
export JIRA_TOKEN="<JIRA_TOKEN>"
```

Tambien se puede usar un fichero `.env` con permisos restringidos:

```bash
chmod 600 ~/.config/jira-cli/.env
```
...
```bash
jira_tool.sh --issue TICKET-1
```

JQL a CSV:

```bash
jira_tool.sh --jql 'project = ABC AND created >= startOfMonth(-1)' --out salida.csv
```

Lista de claves a CSV:

```bash
jira_tool.sh --keys claves.txt --out salida.csv
```

## Paginacion

Para endpoints paginados, iterar usando `start` y `limit` hasta que el numero de resultados devuelto sea cero o menor que el limite.
...
- `/tools/scripts/jira_comment_diff.sh.ORIG`
- `/tools/scripts/jira_comment_diff.sh.tiempo_respuesta_primer_comentario`
- `/tools/scripts/jira_comment_diff.sh_contenido_primer_comentario`
- `/tools/scripts/jira_tool.sh`
- `/tools/scripts/jira_tool.sh.nokeys`
- `/tools/scripts/jira-test.sh`
- `/tools/scripts/jira/get_jsm_keys.sh`
- `/tools/scripts/jira/get_jsm_keys.sh_no_date_range`
